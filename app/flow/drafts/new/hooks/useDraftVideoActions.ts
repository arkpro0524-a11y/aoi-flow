...
export default function useDraftVideoActions(params: Params) {
  ...
  async function extractProductVideoClip() {
     ...
     *
     * 理由:
     * - 動画タブで選んだ背景は videoBackgroundImageUrl に保存される
     * - 以前は composite / ai / bg / template の順で見ていたため、
...
const chromaPreviewVideoUrl = String(j?.chromaPreviewVideoUrl || "").trim();
