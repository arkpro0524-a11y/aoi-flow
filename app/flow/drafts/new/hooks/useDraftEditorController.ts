...
/**
 * 数値を指定範囲に収める補助関数
 * - このファイル内でも背景ズームを 0.5〜4.4 に制限するために使う
 */
...
export default function useDraftEditorController(params: Params) {
  ...
  const imageActions = useDraftImageActions({
     ...
     * 注意：
     * - 先に state を展開する
     * - その後で backgroundScale / backgroundX / backgroundY を上書きする
     * - 逆順にすると、state 側の古い 3.0 上限値で戻される可能性がある
     */
    ...(state as any),

    backgroundScale:
      typeof stateAny.backgroundScale === "number"
        ? clamp(stateAny.backgroundScale, 0.5, 4.4)
        : 1,
    backgroundX:
      typeof stateAny.backgroundX === "number"
        ? clamp(stateAny.backgroundX, -2, 2)
        : 0,
    backgroundY:
      typeof stateAny.backgroundY === "number"
        ? clamp(stateAny.backgroundY, -2, 2)
        : 0,
...
backgroundScale: clamp(
  Number(toFiniteNumberOrUndefined(stateAny.backgroundScale) ?? 1),
  0.5,
  4.4
),
backgroundX: Number(
...
/**
 * overlay preview のリアルタイム再生成を止める
 *
 * 重要:
 * - 以前は slider を動かすたびに
 ...
 *
 * 今回:
 * - overlay内容そのものが変わった時だけ再生成
 * - placement / shadow / background slider 変更では
 *   overlay preview を再生成しない
 *
 * 効果:
 * - slider操作がかなり滑らかになる
 * - React再レンダリング負荷を大幅削減
...
useEffect(() => {
  ...
  return {
    ...
    saveCurrentCaptionSet: (captionActions as any).saveCurrentCaptionSet,
    restoreCaptionSet: (captionActions as any).restoreCaptionSet,
    clearCurrentCaptions: (captionActions as any).clearCurrentCaptions,
