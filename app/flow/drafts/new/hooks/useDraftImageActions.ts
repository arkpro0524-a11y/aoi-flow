...
ctx.filter = "none";
   ...
   *
   * 重要:
   * - BaseImagePanel の「文字サイズ」「文字の上下位置」「文字背景の濃さ」を反映する
   * - 既存の保存形式 textOverlayBySlot[slot] は変えない
   * - 画像生成・合成・アップロード処理には触らない
   */
  ...
  if (overlayText) {
    const fontPx = Math.max(10, Math.round(Number(slotOverlay?.fontSize ?? 64)));
    const lineH = Math.max(10, Math.round(Number(slotOverlay?.lineHeight ?? 1.25) * fontPx));
    ...
    const overlayX = clamp(Number((slotOverlay as any)?.x ?? 50), 0, 100);
    const overlayY = clamp(Number(slotOverlay?.y ?? 70), 0, 100);

    const textCenterX = SIZE * (overlayX / 100);
    const textCenterY = SIZE * (overlayY / 100);

    const textBlockHeight = lines.length * lineH;
    const textStartY = textCenterY - textBlockHeight / 2;

    ctx.font = `900 ${fontPx}px system-ui, -apple-system, BlinkMacSystemFont, "Hiragino Sans", "Yu Gothic", sans-serif`;
    ctx.textBaseline = "top";
    ctx.textAlign = "center";

    const backgroundEnabled = slotOverlay?.background?.enabled !== false;
    const backgroundColor = String(slotOverlay?.background?.color || "rgba(0,0,0,0.45)");

    if (backgroundEnabled) {
      const textWidths = lines.map((line) => ctx.measureText(line).width);
      const maxTextWidth = Math.max(...textWidths, 1);

      const paddingX = fontPx * 0.75;
      const paddingY = fontPx * 0.45;

      const boxWidth = Math.min(SIZE * 0.92, maxTextWidth + paddingX * 2);
      const boxHeight = textBlockHeight + paddingY * 2;

      const boxX = Math.max(0, Math.min(SIZE - boxWidth, textCenterX - boxWidth / 2));
      const boxY = Math.max(0, Math.min(SIZE - boxHeight, textStartY - paddingY));

      ctx.fillStyle = backgroundColor;
      ctx.beginPath();
      ctx.roundRect(boxX, boxY, boxWidth, boxHeight, fontPx * 0.35);
      ctx.fill();
    }

    for (let index = 0; index < lines.length; index++) {
      const line = lines[index];
      const y = textStartY + index * lineH;

      ctx.lineWidth = Math.max(3, Math.round(fontPx * 0.08));
      ctx.strokeStyle = "rgba(0,0,0,0.85)";
      ctx.fillStyle = String((slotOverlay as any)?.color || "#FFFFFF");
 ...
 *
 * リアルタイムプレビューでは使用しません。
 * バー操作中に毎回 canvas を再生成すると重くなるため、
 * 通常表示は CSS Overlay 側で即時反映します。
 *
 * 実際の画像焼き込みは、
 * 「保存」ボタンを押した時だけ実行します。
 */
...
async function saveCompositeTextImageFromCompositeSlot() {
   ...
   */
  async function generateBackgroundImage(keyword: string, referenceBackgroundImageUrl?: string): Promise<string> {
    if (!uid) {
      throw new Error("no uid");
    }
    ...
    try {
      ...
      const body = {
        ...
        backgroundReferenceImageUrl: String(referenceBackgroundImageUrl || "").trim(),
        productCategory: understanding.productCategory,
        productSize: understanding.productSize,
        groundingType: understanding.groundingType,
        sellDirection: understanding.sellDirection,
      ...
      const meta = {
        kind: "bg_only",
        label: "② 背景のみ：希望背景から生成",
        detail: `希望背景=${k}${referenceBackgroundImageUrl ? " / 参考画像あり" : ""}`,
        usedVision: vision,
        purpose: String(staticPurpose ?? ""),
        bgScene: String(understanding.bgScene ?? ""),
        at: Date.now(),
...
background: {
  scale: clamp(Number(backgroundScale ?? 1), 0.5, 4.4),
  x: clamp(Number(backgroundX ?? 0), -2, 2),
  y: clamp(Number(backgroundY ?? 0), -2, 2),
},
      };
...
const compositePatch = {
  ...
  async function syncBgImagesFromStorage() {
    ...
    // 背景履歴の同期だけを目的に、空の下書きを新規作成しない。
    // 新規作成直後は draftId がないため、その場合は何もせず終了する。
    // 画像アップロード・生成・明示保存など、ユーザー操作で下書きIDが作られた後だけ同期する。
