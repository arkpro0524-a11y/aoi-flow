...
function normalizeSavedCaptionSets(input: unknown): NonNullable<DraftDoc["savedCaptionSets"]> {
  if (!Array.isArray(input)) return [];
  ...
  return input
    .map((item: any) => ({
      ...
      ig: normalizeString(item?.ig),
      x: normalizeString(item?.x),
      ig3: Array.isArray(item?.ig3)
        ? item.ig3.map((v: unknown) => String(v ?? "")).slice(0, 3)
        : [],
      instagramSales: normalizeString(item?.instagramSales),
      xSales: normalizeString(item?.xSales),
      ecTitle: normalizeString(item?.ecTitle),
      ecDescription: normalizeString(item?.ecDescription),
      ecBullets: Array.isArray(item?.ecBullets)
        ? item.ecBullets
...
function buildFormalPatch(next: DraftDoc) {
  ...
  return {
    ...
    instagramSales: next.instagramSales,
    xSales: next.xSales,
    ecTitle: next.ecTitle,
    ecDescription: next.ecDescription,
    ecBullets: next.ecBullets,
    savedCaptionSets: normalizeSavedCaptionSets((next as any).savedCaptionSets),
    ...
    compositeTextImageUrl: (next as any).compositeTextImageUrl,
    compositeTextImageUrls: (next as any).compositeTextImageUrls,
    foregroundImageUrl: (next as any).foregroundImageUrl,
    stageImageUrl: (next as any).stageImageUrl,
    ...
    compositeServerPlacementMeta: (next as any).compositeServerPlacementMeta,
    ...
    memo: next.memo,
    ig: typeof next.ig === "string" ? next.ig : igCaption,
    x: typeof next.x === "string" ? next.x : xCaption,
    ig3: Array.isArray(next.ig3) ? next.ig3 : toIg3(shortCopies),
    ...
    (async () => {
      ...
      try {
        ...
        const nextDraft: DraftDoc = {
          ...
          ig: normalizeString((data as any).ig) || igCaption,
          x: normalizeString((data as any).x) || xCaption,
          ig3: Array.isArray((data as any).ig3) ? (data as any).ig3.map((v: unknown) => String(v ?? "")).slice(0, 3) : toIg3(shortCopies),
          instagramSales: normalizeString((data as any).instagramSales),
          xSales: normalizeString((data as any).xSales),
          ecTitle: normalizeString((data as any).ecTitle),
          ecDescription: normalizeString((data as any).ecDescription),
          ecBullets: Array.isArray((data as any).ecBullets)
            ? (data as any).ecBullets.map((v: unknown) => String(v ?? "").trim()).filter(Boolean).slice(0, 5)
            : [],
          savedCaptionSets: normalizeSavedCaptionSets((data as any).savedCaptionSets),
