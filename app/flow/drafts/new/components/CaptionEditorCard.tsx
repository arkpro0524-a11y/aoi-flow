...
type Props = {
  ...
  onSaveCaptionSet?: () => void | Promise<void>;
  onRestoreCaptionSet?: (id: string) => void | Promise<void>;
  onClearCurrentCaptions?: () => void | Promise<void>;
      ...
      <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Btn
              variant="secondary"
              disabled={!uid || busy || !onSaveCaptionSet}
              onClick={() => onSaveCaptionSet?.()}
            >
              現在の文章を保管
            </Btn>
            ...
            <Btn
              variant="ghost"
              disabled={!uid || busy || !onClearCurrentCaptions}
              onClick={() => onClearCurrentCaptions?.()}
            >
              文章欄をクリア
            </Btn>
          </div>
        ...
        <div className="mt-3">
          <select
            value=""
            disabled={busy || !onRestoreCaptionSet || !(d.savedCaptionSets ?? []).length}
            onChange={(e) => {
              const id = e.target.value;
              if (!id) return;
              void onRestoreCaptionSet?.(id);
            }}
            className="w-full rounded-xl border p-2 outline-none"
            style={formStyle}
          >
            <option value="">
              {(d.savedCaptionSets ?? []).length
                ? "保存済み文章を選択"
                : "保存済み文章はまだありません"}
            </option>
            {(d.savedCaptionSets ?? []).map((item) => (
              <option key={item.id} value={item.id}>
                {item.label || new Date(item.createdAt).toLocaleString("ja-JP")}
              </option>
            ))}
          </select>
        </div>
