  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/25 p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="font-black text-white/95">{props.title}</div>
          {props.sub ? <div className="text-sm text-white/65 mt-1">{props.sub}</div> : null}
        </div>
      </div>
      {props.children}
    </div>
...
function HelpBox() {
  return (
    <div className="rounded-2xl border border-white/12 bg-black/35 p-4 space-y-3">
      <div className="text-xl font-black">設定（Brands）— 目次 / 解説</div>

      <div className="text-sm text-white/75 leading-relaxed">
        ここで入れた内容が <b>AI生成（文章 / 画像）</b> に反映されます。<br />
        迷ったら「基本（必須）」だけ埋めればOK。残りは必要になった時だけ使います。
      </div>

      <div className="rounded-xl border border-white/12 bg-black/25 p-3 text-sm text-white/75 space-y-2">
        <div className="font-black text-white/90">✅ AIに反映される項目</div>
        <div className="leading-relaxed">
          <b>文章生成</b>：思想（voiceText）/ IG目的 / X目的 / 禁止（ban）/ must / toneDefault<br />
          <b>画像生成</b>：styleText / rules ＋（今回から）思想（voiceText）を短く入れてブランドに寄せる
        </div>
      </div>

      <div className="rounded-xl border border-white/12 bg-black/25 p-3 text-sm text-white/75 space-y-2">
        <div className="font-black text-white/90">🧩 各欄の意味（混乱防止）</div>
        <ul className="list-disc pl-5 space-y-1">
          <li>
            <b>思想（voiceText）</b>：ブランドの“文章の人格”。ここが空だと、ブランドらしさが出ません（必須）。
          </li>
          <li>
            <b>IG目的 / X目的</b>：媒体ごとの役割。長文寄り/短文寄りなどの方向性を固定します。
          </li>
          <li>
            <b>ban（禁止）</b>：煽りや広告臭を止める安全装置。迷っても入れておく価値が高いです。
          </li>
          <li>
            <b>must</b>：必ず入れたい要素（少数でOK）。入れすぎると文章が固くなります。
          </li>
          <li>
            <b>toneDefault</b>：文章のテンションの初期値（上級者用）。空でも動きます。
          </li>
          <li>
            <b>styleText / rules</b>：画像の雰囲気指定と禁止事項（画像生成するなら必要）。
          </li>
        </ul>
      </div>
    </div>
  ...
  return (
    <div className="p-6 text-white/90 space-y-4">
      ...
      <Card
        ...
        sub={
          <>
            vento / riva の最初の雛形を作ります。後から思想・目的・禁止をあなた用に調整してください。
          </>
        }
      >
        <button
      ...
      <Card
        ...
        sub={
          <>
            まずIDと表示名だけ作成 → 「編集」で思想（必須）と目的を入れる、の流れです。
          </>
        }
      >
        <div className="grid gap-2 max-w-xl">
          ...
          <button
            onClick={createBrand}
            className="rounded-full px-4 py-2 bg-white text-black font-black w-fit"
          >
            作成
          </button>
      ...
      <Card title="一覧" sub="ACTIVEなブランドが /flow/drafts/new の選択肢になります。">
        {loading ? (
          ...
          <div className="text-white/70">まだありません（seedを押すか新規作成してください）</div>
        ) : (
      ...
      {/* 編集UI */}
      {editingId && form ? (
        <div className="rounded-2xl border border-white/12 bg-black/25 p-4 space-y-4">
          <div className="text-xl font-black">編集：{editingId}</div>

          {/* 基本（必須） */}
          <div className="rounded-2xl border border-white/12 bg-black/20 p-4 space-y-3">
            <div className="font-black text-white/95">基本（必須）</div>

            <div className="grid gap-2 max-w-3xl">
              <div className="text-sm text-white/80 font-bold">表示名</div>
              <input
                className="rounded-xl border border-white/15 bg-black/40 px-3 py-2"
                value={form.name}
              ...
              <div className="text-sm text-white/80 font-bold mt-3">
                思想（voiceText）※必須 / 文章と画像に反映
              </div>
              <textarea
                className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 min-h-[110px]"
              ...
              <div className="grid md:grid-cols-2 gap-3 mt-3">
                <div>
                  <div className="text-sm text-white/80 font-bold">IGの目的</div>
                  <textarea
                    className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 min-h-[70px] w-full"
                ...
                <div>
                  <div className="text-sm text-white/80 font-bold">Xの目的</div>
                  <textarea
                    className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 min-h-[70px] w-full"
          ...
          {/* 安全装置（推奨） */}
          <div className="rounded-2xl border border-white/12 bg-black/20 p-4 space-y-3">
            <div className="font-black text-white/95">文章の安全装置（推奨）</div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <div>
                <div className="text-sm text-white/80 font-bold">ban（禁止 / 1行1つ）</div>
                <textarea
                  className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 min-h-[110px] w-full"
                  ...
                  onChange={(e) =>
                    setForm((p) =>
                      p
                        ? {
                            ...p,
                            captionPolicy: { ...p.captionPolicy, ban: splitLines(e.target.value) },
                          }
                        : p
                    )
                  }
              ...
              <div className="rounded-xl border border-white/10 bg-black/15 p-3 text-sm text-white/70">
                <div className="font-black text-white/85 mb-2">おすすめのban例</div>
                <ul className="list-disc pl-5 space-y-1">
                  <li>煽り</li>
                  <li>過剰な断定</li>
                  <li>誇張</li>
          ...
          <div className="rounded-2xl border border-white/12 bg-black/20 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div className="font-black text-white/95">詳細（must / toneDefault）</div>
              <button
            ...
            {openAdvanced ? (
              <div className="grid gap-3 max-w-3xl">
                <div className="grid md:grid-cols-2 gap-3">
                  <div>
                    <div className="text-sm text-white/80 font-bold">must（必ず入れたい / 1行1つ）</div>
                    <textarea
                      className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 min-h-[110px] w-full"
                      ...
                      onChange={(e) =>
                        setForm((p) =>
                          p
                            ? {
                                ...p,
                                captionPolicy: { ...p.captionPolicy, must: splitLines(e.target.value) },
                              }
                            : p
                        )
                      }
                  ...
                  <div>
                    <div className="text-sm text-white/80 font-bold">toneDefault（上級者用）</div>
                    <input
              ...
              <div className="text-sm text-white/65">※ 普段は不要。必要になったら開いて調整。</div>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-white/12 bg-black/20 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div className="font-black text-white/95">画像（styleText / rules）</div>
              <button
            ...
            {openImage ? (
              <div className="grid gap-3 max-w-3xl">
                ...
                <div>
                  <div className="text-sm text-white/80 font-bold">styleText（雰囲気）</div>
                  <textarea
                    className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 min-h-[80px]"
                ...
                <div>
                  <div className="text-sm text-white/80 font-bold">rules（禁止 / 1行1つ）</div>
                  <textarea
                    className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 min-h-[110px]"
