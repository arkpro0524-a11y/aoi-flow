...
function normalizePlacement(input: unknown): PlacementInput {
  ...
  return {
    ...
    background: {
      scale: clamp(Number(backgroundRaw.scale ?? 1), 0.5, 4.4),
      x: clamp(Number(backgroundRaw.x ?? 0), -2, 2),
      y: clamp(Number(backgroundRaw.y ?? 0), -2, 2),
    },
  };
}
...
async function transformBackground(
  ...
  const canvas = 1024;
  const bgScale = clamp(Number(background?.scale ?? 1), 0.5, 4.4);
