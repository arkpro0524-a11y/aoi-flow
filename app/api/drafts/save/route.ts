...
function normalizePatch(input: Record<string, unknown>, uid: string): JsonLikeObject {
  ...
  // 下書き一覧は phase == "draft" で検索するため、
  // 新規作成時・通常保存時のどちらでも必ず draft を保存する。
  // ここが空だと、保存は成功しているのに一覧に出ない。
  patch.phase = "draft";

  // 古い画面側は brand を見ることがあるため、brandId と同じ値も保持する。
  if ("brand" in input) {
    patch.brand = normalizeBrandId(input.brand);
  } else if ("brandId" in input) {
    patch.brand = normalizeBrandId(input.brandId);
  }
   ...
   * AOI FLOW 完成保存の本質修正
   *
   * これまでこのAPIは正式保存対象が少なすぎたため、
   * フロント側では保存成功に見えても、Firestoreには
   * ・生成文章の保管データ
   * ・文字焼き込み画像URL
   * ・合成済み完成画像URL
   * ・前景切り抜きURL
   ...
   * などが落ちていました。
   *
   * ここでは「フロントの下書き編集で使う安全なキー」をまとめて保存対象にします。
   * null / undefined は保存しないため、Firestoreに空データが残りにくくなります。
   */
  const passThroughKeys = [
    ...
    "staticImageLogs",
    "staticImageVariants",
    "selectedStaticVariantId",
    "selectedStaticPrompt",
    "selectedStaticVariantTitle",
    "imagePurpose",
    "bgCandidates",
    "selectedBgCandidateId",
    "bgPickLogs",
    "bgRefinedPrompt",
    "bgRefinedUrl",
    "bgRefineEnabled",
  ...
  // 一覧表示の検索条件を必ず満たす。
  // 途中のpassThroughで上書きされても、最後に draft へ戻す。
  patch.phase = "draft";
