...
function normalizeLog(id: string, data: any) {
  ...
  const source = normalizeSellCheckSource(data.source || data.imageSource);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const listingStatus =
      patch.listingStatus === undefined && patch.sold === undefined
        ? undefined
        : normalizeListingStatus({
            sold: patch.sold,
    ...
    const source =
      patch.source === undefined && patch.imageSource === undefined
        ? undefined
        : normalizeSellCheckSource(patch.source || patch.imageSource);
    ...
    const payload = cleanObject({
      ...
      sold:
        listingStatus !== undefined
          ? listingStatus === "sold"
          : patch.sold === undefined
          ? undefined
          : safeBoolean(patch.sold),
       ...
       * 少数データ判定用
       */
      productType: safeString(patch.productType) || undefined,
      characterName: safeString(patch.characterName) || undefined,
      seriesName: safeString(patch.seriesName) || undefined,
      maker: safeString(patch.maker) || undefined,
      era: safeString(patch.era) || undefined,
      collectorGenre: safeString(patch.collectorGenre) || undefined,
      materialType: safeString(patch.materialType) || undefined,
      extractedKeywords:
        patch.extractedKeywords === undefined
          ? undefined
          : safeStringArray(patch.extractedKeywords),
      ...
      rarityScore: safeScore(patch.rarityScore),
      demandScore: safeScore(patch.demandScore),
      brandPowerScore: safeScore(patch.brandPowerScore),
      collectorScore: safeScore(patch.collectorScore),
      ageValueScore: safeScore(patch.ageValueScore),
      trendScore: safeScore(patch.trendScore),
      marketSupplyScore: safeScore(patch.marketSupplyScore),
      keywordStrength: safeScore(patch.keywordStrength),
      rareReasons:
        patch.rareReasons === undefined ? undefined : safeStringArray(patch.rareReasons),

      score: safeScore(patch.score),
      rank: safeString(patch.rank) || undefined,

      hasImage:
        patch.hasImage === undefined ? undefined : safeBoolean(patch.hasImage),
      imageUrl: safeString(patch.imageUrl) || undefined,
      imageFileName: safeString(patch.imageFileName) || undefined,
      imageFileSize: safeNumber(patch.imageFileSize),
