...
const AI_BG_VERSION = "v28_user_requested_background_priority";
...
function classifyKeywordScenario(keyword: string): KeywordScenario {
  ...
  if (
    k.includes("コレクションケース") ||
    k.includes("ショーケース") ||
    k.includes("飾り棚") ||
    k.includes("display case") ||
    k.includes("collection case") ||
    k.includes("showcase")
  ) {
    return "display_case";
  }
...
function resolveGenerationContext(args: {
  ...
  const isDisplayCase = keywordScenario === "display_case";
...
extraSceneHints: [
  ...
  if (keywordScenario === "display_case") {
    return {
      scenario: keywordScenario,
      title: "collection display case",
      upperRole: "front-facing transparent display case top frame or soft shelf line",
      leftRole: "subtle transparent side panel or acrylic edge",
      rightRole: "subtle transparent side panel or acrylic edge",
      lowerRole: "clean empty shelf base inside the display case",
      allowedStructures: [
        "front-facing empty acrylic display case",
        "glass showcase interior",
        "clean collectible shelf",
        "transparent side panels",
        "soft gray low-glare acrylic surface",
      ],
      forbiddenAccidents: [
        "normal room",
        "doorway",
        "corridor",
        "living room",
        "bright white empty wall",
        "glare",
        "displayed product already inside",
      ],
      preferredMaterials: [
        "low-glare acrylic",
        "soft gray transparent glass",
        "pale matte shelf",
        "subtle reflections",
      ],
      extraSceneHints: [
        "The image should feel like looking into an empty collection case.",
        "Do not create a room background when the request says collection case.",
        "The center shelf must remain empty for the later product.",
      ],
    };
  }
...
"Do not let any object become the visual subject.",
  ...
  if (keywordScenario === "display_case") {
    return [
      "User requested background is mandatory: collection case / display case.",
      "Generate an empty front-facing collection display case or showcase, not a room.",
      "The viewer must immediately understand this is an empty display case interior from transparent side panels, shelf base, and subtle acrylic/glass reflections.",
      "Do not create doorway, corridor, living room, white wall, floor-room, or generic studio wall.",
      "Avoid strong glare and avoid pure white blown highlights.",
      "Keep the central shelf area empty for later product placement.",
    ];
  }
...
function buildPrompt(args: {
  ...
  const keywordRequirement = [
    "USER REQUESTED BACKGROUND IS MANDATORY:",
    `- User requested background: ${keyword}`,
    "- Follow this requested background literally unless it would create the product itself.",
    "- The requested background is not optional flavor.",
    "- The final scene must clearly reflect the requested background as the main background type.",
  ...
  return [
    ...
    "",
    referenceStyleText
      ? "REFERENCE IMAGE / SCREENSHOT GUIDANCE:"
      : "",
    referenceStyleText
      ? "- A user supplied a reference image or screenshot for the desired background mood."
      : "",
    referenceStyleText
      ? `- Use this reference analysis for brightness, color tone, surface/plainness, and amount of empty space: ${referenceStyleText}`
      : "",
    referenceStyleText
      ? "- Combine the reference guidance with the written requested background. Do not copy any product, logo, text, or person from the reference."
      : "",
    referenceStyleText ? "" : "",
    ...keywordRequirement,
    "",
...
"- Reject images where the center becomes a square slab or flat glowing plate.",
    ...
    "The user requested background must be visible from structure, not guessed from atmosphere.",
    "",
    "Additional rules:",
    ...mergedRules.map((r) => `- ${r}`),
  ]
...
async function analyzeReferenceBackgroundImage(url: string): Promise<string> {
  ...
  try {
    ...
    const meta = await sharp(buf, { failOn: "none" }).metadata();
    const stat = await sharp(buf, { failOn: "none" })
      .resize(64, 64, { fit: "inside" })
      .removeAlpha()
      .stats();

    const channels = stat.channels || [];
    const means = channels.slice(0, 3).map((c) => Math.round(Number(c.mean || 0)));
    const avg = means.length ? Math.round(means.reduce((a, b) => a + b, 0) / means.length) : 0;
    const maxDiff = means.length
      ? Math.max(...means) - Math.min(...means)
      : 0;

    const brightness = avg >= 205 ? "very bright" : avg >= 175 ? "bright" : avg >= 135 ? "medium bright" : "dark";
    const colorTone = maxDiff <= 10
      ? "neutral gray/white tone"
      : means[0] >= means[2] + 12
      ? "warm beige/wood-like tone"
      : means[2] >= means[0] + 12
      ? "cool blue/gray tone"
      : "soft neutral tone";

    const orientation = Number(meta.width || 0) > Number(meta.height || 0) * 1.15
      ? "wide reference"
      : Number(meta.height || 0) > Number(meta.width || 0) * 1.15
      ? "vertical reference"
      : "square-like reference";

    return [
      `reference brightness: ${brightness}`,
      `reference color tone: ${colorTone}`,
      `reference format: ${orientation}`,
      "keep similar cleanliness, visual quietness, and amount of empty placement space",
      "avoid copying any object, product, character, logo, readable text, or exact layout",
    ].join("; ");
  } catch {
    return "";
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const backgroundReferenceImageUrl = validateReferenceImageUrl(body.backgroundReferenceImageUrl);
    const referenceStyleText = await analyzeReferenceBackgroundImage(backgroundReferenceImageUrl);
