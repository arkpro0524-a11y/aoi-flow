...
function MobileMenuCard({ item }: { item: MenuItem }) {
  return (
    <Link
      href={item.href}
      className="group flex min-h-[158px] flex-col rounded-[1.25rem] border border-white/70 bg-white/58 px-4 py-4 text-left shadow-[0_16px_34px_rgba(15,30,48,0.10)] backdrop-blur-md active:scale-[0.98]"
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <span className="mt-3 text-3xl font-light leading-none text-[#1c4f82]">›</span>
      </div>

      <div className="mt-auto">
        <h2 className="text-[17px] font-black leading-tight tracking-[0.04em] text-[#0f3765]">
          {item.label}
        </h2>
        <p className="mt-2 text-[12px] font-semibold leading-relaxed text-[#0f1e30]/76">
          {item.description}
        </p>
      </div>
    </Link>
...
function MobileHomeView({ onLogout }: { onLogout: () => void }) {
  return (
    <main className="min-h-screen bg-[#eaf3fb] text-[#0f1e30]">
...
export default function HomePage() {
  ...
  useEffect(() => {
    /*
      PC画面はブラウザ幅で切り替えない。
      デスクトップのウィンドウを狭くしても既存PCトップを維持し、
      iPhone / Android などのスマホ端末だけ専用トップを表示する。
    */
    const ua = navigator.userAgent.toLowerCase();
    setIsMobileDevice(/iphone|ipod|android.*mobile/.test(ua));
  }, []);
  ...
  if (isMobileDevice) {
    return <MobileHomeView onLogout={logout} />;
  }
