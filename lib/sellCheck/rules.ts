...
export function normalizeSellCheckSource(value: unknown): SellCheckSource {
  ...
  if (s === "yahoo_shopping") return "yahoo_shopping";
...
export function normalizeListingStatus(args: {
  sold: unknown;
  listingStatus?: unknown;
}): SellCheckListingStatus {
  const status = String(args.listingStatus ?? "").trim();

  if (status === "sold") return "sold";
  if (status === "active") return "active";
  if (status === "unknown") return "unknown";

  const soldValue = args.sold;
