...
async function uploadProductVideo(file: File) {
  ...
  try {
    ...
    if (file.size > MAX_SIZE) {
      throw new Error("動画サイズが大きすぎます（300MB以下）");
    }
    ...
    // =========================
    // Firebase Storage 保存先
    // =========================
    const path =
      `users/${props.uid}/drafts/${props.draftId}/source/${Date.now()}.${ext}`;
    ...
    if (!url) {
      throw new Error("動画URL取得に失敗しました");
    }

    // =========================
    // 親へ保存通知
    // =========================
await props.onSaveSourceVideo?.({
  url,
  path,
});
...
if (!backgroundImageUrl) {
  throw new Error("背景画像が未選択です");
}

if (typeof props.onExtractProductVideoClip !== "function") {
  throw new Error("切り抜き関数が未接続です");
}

setMsg("🎬 動画切り抜き中...");

await props.onExtractProductVideoClip({
  sourceVideoUrl: url,
  backgroundImageUrl,
});

setMsg("✅ 動画切り抜き＋背景合成が完了しました");
  } catch (e: any) {
    props.setReason(
      e?.message || "商品撮影動画の保存に失敗しました"
    );
  } finally {
    setLocalBusy(false);
  }
